...
class IngredientesXRecetaService extends ResolversOperationsService
{    
    collection = COLLECTIONS.INGREDIENTES_X_RECETA;
    ...
    async insert()
    {
        ...
        {
            status: false, 
            message: 'La información para la ingredientesxreceta no es correcta.',
            ingredientesxreceta:  {} || null,
        };
        respuesta.ingredientesxreceta = null;
        ...
        if(!datosAcceso.status)
        {
            ...
            //     {
            //         respuesta.message = `La ingredientesxreceta ${ficha.nombre} ya existe en la base de datos`;
            //     }
            //     else
            //     {
    ...
    async items()
    {
        ...
        const respuesta = await this.list(this.collection, 'ingredientesxreceta', {}, page, itemsPage);

        return {info: respuesta.info, status: respuesta.status, message: respuesta.message, ingredientesxreceta: respuesta.items};
    }
    ...
    async details()
    {
        ...
        return {status: respuesta.status, message: respuesta.message, ingredientesxreceta: respuesta.item};
    }
    ...
    async modify()
    {
        ...
        // solo los administradores pueden modificar ingredientesXReceta.
        const datosAcceso = tengoPermisos(this.getContext().token!, PERFILES.ADMIN);
        if(datosAcceso.status)
        {

            // TO DO: hay que buscar por los dos IDs
    ...
    async delete()
    {
        ...
        {
            status: false, 
            message: 'La información para el borrado de la ingredientesxreceta no es correcta.',
            ingredientesxreceta:  {} || null,
        };
        respuesta.ingredientesxreceta = null;
...
export default IngredientesXRecetaService;